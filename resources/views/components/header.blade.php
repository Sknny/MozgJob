<header class="bg-blue-900 text-white p-4" x-data="{open: false}">
    <div class="container mx-auto flex justify-between items-center">
        ...
        <nav class="hidden md:flex items-center space-x-4">
            ...
            @auth
            <x-nav-link url='/jobs/saved' :active="request()->is('saved')">Saved Jobs</x-nav-link>
            <x-nav-link url='/dashboard' :active="request()->is('dashboard')" icon="gauge">Dashboard</x-nav-link>
            <x-logout-button />
            <x-button-link url='/jobs/create' :active="request()->is('create')" icon="edit">Create Job</x-button-link>
            @else
    ...
    >
        ...
        <x-nav-link url='/jobs/saved' :active="request()->is('saved')" :mobile="true">Saved Jobs</x-nav-link>
        <x-nav-link url='/dashboard' :active="request()->is('dashboard')" icon="gauge" :mobile="true">Dashboard</x-nav-link>
        <x-logout-button />
        <div class="pt-2"></div>
        <x-button-link url='/jobs/create' :active="request()->is('create')" :block="true" icon="edit">Create Job</x-button-link>
        @else
